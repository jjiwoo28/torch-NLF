...
if __name__ == '__main__':
    ...
    poses = np.load(os.path.join(opt.path, 'poses.npy'))
    bounds = np.load(os.path.join(opt.path, 'bounds.npy'))
    N = poses.shape[0]
    
    #breakpoint()
    print(f'[INFO] loaded {len(images)} images, {N} poses_bounds as {poses.shape}')
