...
from nerf.neulf.network_feature_split import FreqAndSHNeuLFNetwork,WireAndSHNeuLFNetwork,FreqHNeuLFNetwork
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wire_and_sh', action='store_true')
    parser.add_argument('--freq_neulf', action='store_true')
    ...
    if opt.wire_and_sh:
        model = WireAndSHNeuLFNetwork( num_layers=opt.depth + 2 ,hidden_dim=opt.width , act = opt.act, input_dim=input_dim)
    elif opt.freq_neulf:
        model = FreqHNeuLFNetwork( num_layers=opt.depth + 2 ,hidden_dim=opt.width , act = opt.act , input_dim=input_dim)
    else:
        model = FreqAndSHNeuLFNetwork( num_layers=opt.depth + 2 ,hidden_dim=opt.width , act = opt.act, input_dim=input_dim)
    
    # if opt.neulf:
    #     model = NeuLFNetwork( num_layers=opt.depth + 2 ,hidden_dim=opt.width)
    ...
    if opt.test:
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, scheduler_update_every_step=False, metrics=metrics, use_checkpoint=opt.ckpt, eval_interval=opt.eval_interval)
