...
@torch.cuda.amp.autocast(enabled=False)
def get_rays2(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1):
    ...
    i, j = custom_meshgrid(torch.linspace(0, W-1, W, device=device), torch.linspace(0, H-1, H, device=device)) # float
    i = i.t().expand([B, H, W]) + 0.5
    j = j.t().expand([B, H, W]) + 0.5
...
@torch.cuda.amp.autocast(enabled=False)
def get_uvxy(poses, intrinsics, H, W):
    device = poses.device
    B = poses.shape[0]

    aspect = W / H
    u = torch.linspace(-1, 1, W, dtype=torch.float32, device=device)
    v = torch.linspace(1, -1, H, dtype=torch.float32, device=device) / aspect
    ...
    s = torch.ones_like(u) * poses[:, 0, 3].view(B, 1, 1)  # Use view for safe broadcasting
    t = torch.ones_like(v) * poses[:, 1, 3].view(B, 1, 1)

    uvst = torch.stack((u, v, s, t), dim=-1)

    return uvst

def get_uvxy_new(poses, intrinsics, H, W):
    pass

def get_uvst(poses, intrinsics, H, W , focal_depth):
    B = poses.shape[0]
    rays = get_rays2(poses, intrinsics, H, W,)
    rays_o = rays["rays_o"]
    rays_d = rays["rays_d"]
    ...
    n = H*W *B
    focal_depth  = focal_depth*2
    plane_normal = torch.tensor([0.0, 0.0, 1.0]).expand(n, -1)
    p_uv = torch.tensor([0.0, 0.0, 0.0]).expand(n, -1)
    p_st = torch.tensor([0.0, 0.0, -focal_depth]).expand(n, -1)
    inter_uv = rayPlaneInter(plane_normal,p_uv,rays_o,rays_d)
    inter_st = rayPlaneInter(plane_normal,p_st,rays_o,rays_d)
    
    inter_uvst = torch.cat((inter_uv[:, :2], inter_st[:, :2]), dim=-1).float()
    uvst = inter_uvst.reshape(B, H, W, 4)
    return uvst 
...
def rayPlaneInter(n, p0, ray_o, ray_d):
    ...
    dist = (s1 - s2) / s3
    dist_group = dist.unsqueeze(1).expand(-1, 3)
...
class Trainer(object):
    def __init__(self, 
        ...
        if lr_scheduler is None:
            ...
            self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=0.999)  # fake scheduler
        else:
    ...
    def train_step(self, data):
        ...
        # MSE loss
        loss = self.loss_coeff* self.criterion(pred_rgb, gt_rgb).mean(-1) # [B, N, 3] --> [B, N]
    ...
    def train_one_epoch(self, loader):
        ...
        if self.local_rank == 0:
            pbar = tqdm.tqdm(total=len(loader) * loader.batch_size, bar_format='{desc}: {percentage:3.0f}% {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]',dynamic_ncols=True)
